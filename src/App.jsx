...
export default function App() {
  ...
  const [breaksTaken, setBreaksTaken] = useState(() => {
    try {
      ...
      const savedDate = localStorage.getItem('breaksTakenDate');
      const today = new Date().toDateString();
      if (savedDate !== today) return 0;
      return saved ? parseInt(saved) : 0;
    } catch { return 0; }
  ...
  const [dailyHabits, setDailyHabits] = useState(() => {
    try {
      ...
      if (savedDate !== today) {
        return { sleep: false, nature: false, reading: false, gratitude: false, meditation: false };
      }
      return saved ? JSON.parse(saved) : { sleep: false, nature: false, reading: false, gratitude: false, meditation: false };
    } catch { return { sleep: false, nature: false, reading: false, gratitude: false, meditation: false }; }
  });
  ...
  // --- Stare Provocări (PERSISTENT) ---
  const [challengeConfig, setChallengeConfig] = useState(() => {
    try {
      const saved = localStorage.getItem('challengeConfig');
      return saved ? JSON.parse(saved) : { name: "", reward: "", isConfigured: false };
    } catch { return { name: "", reward: "", isConfigured: false }; }
  });
  ...
  useEffect(() => {
    ...
    localStorage.setItem('breaksTakenDate', new Date().toDateString());
  }, [breaksTaken]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem('dailyHabitsDate', new Date().toDateString());
  }, [dailyHabits]);
